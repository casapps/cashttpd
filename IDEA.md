...
and get a real HTTP/1.1-and-newer server with `.htaccess`/`.htpasswd` compatibility, CGI support,
...
- Serves a directory tree over HTTP and HTTPS, RFC-compliant (HTTP/1.1 at minimum; conditional
  requests, range requests, correct status codes, correct header semantics).
...
| `--config-test` / `-t` | — (CLI-only) | `false` | Parses and validates the effective config, prints errors to stderr, exits 0 (valid) or 1 (invalid); never binds sockets or starts the server (AI.md → PART 14 "Signals & Lifecycle", required because this project is an RFC-compliant HTTP/1.1+ server). |
...
- RFC-compliant HTTP/1.1 at minimum (correct status codes, conditional/range requests, header
  semantics) — see "Core behavior."
